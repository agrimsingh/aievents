...
- **Events pipeline**: Curated **`sourceUrl`** entries in **`data/events.ts`** (Luma or other hosts); **`lib/luma.ts`** fetches each page and parses **JSON-LD `Event` / `SocialEvent`** for title, dates, location, image, etc. Optional **`scrapeFallback`** on an entry supplies a snapshot when server-side fetch cannot return usable JSON-LD (e.g. bot/challenge HTML from datacenter IPs). Add **`next/image`** **`images.remotePatterns`** in **`next.config.ts`** for each non-Luma cover host (e.g. imgix) so covers render.
