...
export async function handleSlackIntakeWebhook({
  ...
  try {
    ...
    configuration = {
      ...
      removalReactions: new Set(
        parseList(
          environment.SLACK_EVENT_REMOVAL_REACTIONS?.trim() || "wastebasket",
        ),
      ),
  ...
  const configuredReactions = new Set([
    ...configuration.approvalReactions,
    ...configuration.removalReactions,
  ]);
  const shouldDispatch =
    payload.type === "event_callback" &&
    event?.type === "reaction_added" &&
    configuredReactions.has(event.reaction) &&
    configuration.approverIds.has(event.user) &&
    event.item?.type === "message" &&
    event.item.channel === configuration.channelId &&
    /^\d+\.\d+$/.test(event.item.ts ?? "");
